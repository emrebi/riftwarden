...
| ARTINT-07 | DONE (Gemini Flash High, 1 timeout resume; bg image fitted to field rect + 8px edge stretch for letterbox, fallback flat paint; citadel/portal aspect kept; world atlas 2048; RISK: castle rect y 0.42-0.60 (h 0.18) while slots span y 0.33-0.69 -> defenders may stand outside fortress art; needs device check + possible castle scale/slot realignment round) | ARTINT-07 commit |
...
| UI-CLEAN | DONE (Gemini Flash Medium, 0 fix; AppGradients/AppShadows.glow/panel, AppBorders.subtle, void* removed; 5 usages -> AppColors.background; ui_lint `legacy-token` rule; 8 unused world sprites deleted (rift_violet kept: sectors riftSkin); CLAUDE.md/AGENTS.md font+neon notes updated; boot screen radial gradient -> flat background, check on device) | UI-CLEAN commit |
| QA-FINAL | DONE (Gemini Flash Medium, 0 fix; smoke matrix en/ar/de x 640x360/800x360, 66 smoke / 87 total PASS, 0 skip; DESIGN §30 17 PASS + 1 GAP (device-only art checks, KNOWN_GAPS QA-FINAL section). UI migration code complete; remaining = user device checks) | QA-FINAL commit |
